...
$m = new MongoDBStock();
$rowlist = $m->mongoSearch('test.stocklist', ['code' => '300343'], ['projection' => ['code' => 1, 'name' => 1, '_id' => 0]]); // ['code'=>'300343']

$i = 1;
foreach ($rowlist as $stockCode) {
    //$url_page = 'http://vip.stock.finance.sina.com.cn/corp/go.php/vCI_CirculateStockHolder/stockid/' . $stockCode['code'] . '.phtml'; // $stockCode['code']

    $url_page = 'http://chuansong.me/n/371498951536';
    $string = QueryList::get($url_page)->removeHead()->getHtml();//->encoding('UTF-8', 'GB2312')
    //解析常规概念
//    $table = QueryList::html($string);
//    $tableRows = $table->texts();
//    $arrGN = $tableRows->all();
    echo $i . "\r\n";
    print_r( $string );
//    $arrStockGN = array('code' => $stockCode['code'], 'name' => $stockCode['name'], 'GN' => $arrGN);
//    $m->mongoInsert("test.stockgd", $arrStockGN); // date("Y-m-d")
    /**
